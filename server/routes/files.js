...
router.get("/:id/orders", async (req, res) => {
  ...
  Order.findAll({
    ...
    include: [File, Client, Doctor, Group, Office]
  })
    .then(array => {
...
// eliminar archivo y sus ordenes
router.delete("/delete/:id", (req, res) => {
  let fileId = req.params.id;
  File.destroy({ where: { id: fileId } }).then(() => {
    Order.destroy({ where: { fileId: fileId } }).then(data =>
      res.status(201).json({ error: false, data, msg: "Registro Eliminado" })
    );
  });
});
